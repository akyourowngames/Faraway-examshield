        ...
        <div className="flex items-center gap-2">
          {agent.status === "draft" && (
            <button onClick={handleDeploy} disabled={deploying}
              className="flex items-center gap-2 px-4 py-2 bg-white text-black text-xs font-bold uppercase tracking-widest hover:bg-white/90 transition-colors disabled:opacity-50">
              {deploying ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Rocket className="w-3.5 h-3.5" />}
              {deploying ? "Deploying..." : "Deploy"}
            </button>
          )}
          {agent.status === "deploying" && (
            <span className="flex items-center gap-1.5 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-white/60 bg-white/5 border border-white/10">
              <Loader2 className="w-3 h-3 animate-spin" /> Deploying
            </span>
          )}
          {telegramConfig?.deploymentStatus === "deployed" && agent.status === "active" && (
            <span className="flex items-center gap-1.5 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-emerald-400/80 bg-emerald-400/5 border border-emerald-400/20">
        ...
        {tab === "knowledge" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-semibold uppercase tracking-[0.15em] text-white/50">Knowledge Sources</h3>
              <label className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 text-white/60 text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 hover:border-white/20 transition-colors cursor-pointer">
                {uploadingKnowledge ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plus className="w-3 h-3" />}
                {uploadingKnowledge ? "Uploading..." : "Add Source"}
              ...
              knowledgeSources.map((source) => (
                <div key={source.id} className="border border-white/10 bg-white/[0.02] p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {source.errorMessage && (
                      <div className="text-[10px] text-red-400/70 mt-1">{source.errorMessage}</div>
                    )}
                  </div>
                  ...
                  <button onClick={() => handleDeleteSource(source.id)} disabled={deletingSource === source.id}
                    className="p-1.5 text-white/20 hover:text-red-400 transition-colors disabled:opacity-30">
                    {deletingSource === source.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
                  </button>
                </div>
              ))
            )}
